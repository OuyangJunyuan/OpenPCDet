...
class DataAugmentor(object):
    ...
    def random_make_slope_in_scene(self, data_dict=None, config=None):
        ...
        gt_boxes, points = data_dict['gt_boxes'], data_dict['points']
        gt_boxes_extension = np.zeros([gt_boxes.shape[0], 2])  # for pitch roll
        if choice < prob:
            gt_boxes, points, gt_boxes_extension = augmentor_utils.point_cloud_random_make_slope(
                gt_boxes, points, params=(dist_mean, dist_var, angle_mean, angle_var)
            )
        data_dict['gt_boxes'] = np.concatenate((gt_boxes, gt_boxes_extension), axis=1)
        data_dict['points'] = points
        return data_dict
