...
def point_r_cnn_hook(input_dict=None):
    ...
    if input_dict is not None:
        hook_dict = {'point_cls_scores': copy.deepcopy(input_dict['point_cls_scores']),
                     'point_cls_preds': copy.deepcopy(input_dict['batch_cls_preds']),
                     'points': copy.deepcopy(input_dict['point_coords'])}
    else:
        return hook_dict
...
class PointHeadBox(PointHeadTemplate):
    ...
    def assign_targets(self, input_dict):
        ...
        targets_dict = self.assign_stack_targets(
            ...
            set_ignore_flag=True,  # 忽略（设置为背景值-1）那些只属于拓展box与原box之间的点云
            use_ball_constraint=False, ret_part_labels=False, ret_box_labels=True
        )
    ...
    def forward(self, batch_dict):
        ...
        """
        # USE_POINT_FEATURES_BEFORE_FUSION 即point feature拿到后
