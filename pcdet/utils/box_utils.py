...
def boxes_to_pyramids(boxes):
    """
    Args:
        boxes(N,7)
    Returns:
        pyramids(N, 6(faces), 15=5(corner)*3(point_dim))
    """
    # (6,4)
    ...
    boxes_corners = boxes_to_corners_3d(boxes)  # (N,8,3)
    pyramid_corners = boxes_corners[:, pyramid_orders]  # (N,6,4,3)
    pyramid_centers = boxes[:, None, None, 0:3].repeat(6, axis=1)  # (N,6,1,3)
    pyramids = np.concatenate((pyramid_centers, pyramid_corners), axis=2).reshape((-1, 6, 15))
    return pyramids
...
def points_in_pyramids_mask(points, pyramids):
    ...
    Args:
        points(N,3)
        pyramids(K, 1(faces), 5(corner), 3(point_dim))
    Returns:
        flags(N(bool),K(in_which_pyramid)
    """
    # (N,5,3)
    ...
    for i, pyramid in enumerate(pyramids):
        flags[:, i] = in_hull(points[:, 0:3], pyramid)
    return flags
